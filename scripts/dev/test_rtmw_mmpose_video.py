...
Cách chạy từ root project:
    ...
    python scripts/dev/test_rtmw_mmpose_video.py --video "data/datasets/WLASL/raw/videos/example.mp4"

Có thể chỉ định device:

    python scripts/dev/test_rtmw_mmpose_video.py --video "path/to/video.mp4" --device cuda:0
    python scripts/dev/test_rtmw_mmpose_video.py --video "path/to/video.mp4" --device cpu

Output test được ghi vào:

    scripts/dev/_rtmw_mmpose_video_test_output/
