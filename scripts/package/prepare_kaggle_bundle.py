...
def build_readme_text(
    ...
    config_path = (
        ...
        else "configs/preprocessing/pose/pose_rtmw_l.yaml"
    )
    ...
    return "\n".join(
        [
            ...
            "9. Run:",
            f"   python scripts/preprocess/02_extract_pose_rtmw.py --config {config_path} --startup-only",
            f"   python scripts/preprocess/02_extract_pose_rtmw.py --config {config_path} --smoke-one-frame",
            f"   python scripts/preprocess/02_extract_pose_rtmw.py --config {config_path} --limit 1",
            "",
            "## Notes",
            "",
...
def main() -> int:
    ...
    try:
        ...
        write_readme(
            ...
            subset=validation.subset,
            use_kaggle_config=(
                project_root / "configs" / "preprocessing" / "pose" / "pose_rtmw_l_kaggle.yaml"
            ).exists(),
            no_zip=args.no_zip,
        )
