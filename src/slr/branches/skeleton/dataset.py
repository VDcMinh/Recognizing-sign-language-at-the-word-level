"""Dataset loader utilities for skeleton graph tensors."""

from __future__ import annotations

import ast
from collections.abc import Callable, Iterable, Sequence
from dataclasses import dataclass
from pathlib import Path
from typing import Any

import numpy as np
import pandas as pd
import torch
from torch.utils.data import Dataset

from slr.data.manifests import SKELETON_INPUT_MANIFEST_COLUMNS
from slr.data.validation import require_columns
from slr.utils.io import read_csv, read_yaml
from slr.utils.logging import get_logger


LOGGER = get_logger(__name__)
ALLOWED_SPLITS = ("train", "val", "test")
DEFAULT_DATASET_ROOT_MARKER = "data/datasets/WLASL/"
DEFAULT_SKELETON_BRANCH_MARKER = "branch_inputs/skeleton/rtmw_l/"
DEFAULT_EXPECTED_SHAPES = {
    "selected_27": (3, 150, 27, 1),
    "selected_31": (3, 150, 31, 1),
}
REQUIRED_DATASET_COLUMNS = (
    "sample_id",
    "video_id",
    "gloss",
    "class_id",
    "split",
    "graph_tensor_path",
    "status",
)


@dataclass(frozen=True)
class SkeletonSampleRecord:
    """Resolved manifest row used by :class:`SkeletonGraphDataset`."""

    sample_id: str
    video_id: str
    gloss: str
    class_id: int
    split: str
    graph_tensor_path: Path
    manifest_index: int
    instance_uid: str = ""
    keypoint_set: str = ""
    original_path_text: str = ""
    notes: str = ""


def _normalize_split(value: Any) -> str:
    """Normalize split strings to lowercase ``train/val/test`` tokens."""

    if value is None or pd.isna(value):
        return ""
    return str(value).strip().lower()


def _safe_str(value: Any, default: str = "") -> str:
    """Convert nullable values to stable strings."""

    if value is None:
        return default
    try:
        is_na = pd.isna(value)
    except TypeError:
        is_na = False
    if isinstance(is_na, (bool, np.bool_)) and is_na:
        return default
    return str(value)


def _parse_int(value: Any, field_name: str) -> int:
    """Convert one manifest value to ``int`` with a readable error."""

    if value is None or pd.isna(value):
        raise ValueError(f"{field_name} is missing.")
    try:
        return int(float(value))
    except (TypeError, ValueError) as exc:
        raise ValueError(f"{field_name} must be convertible to int, got {value!r}.") from exc


def _coerce_torch_dtype(value: Any) -> torch.dtype:
    """Resolve user-configured tensor dtypes."""

    if isinstance(value, torch.dtype):
        return value
    if value is None:
        return torch.float32
    text = str(value).strip().lower()
    mapping = {
        "float32": torch.float32,
        "torch.float32": torch.float32,
        "float": torch.float32,
        "fp32": torch.float32,
        "float64": torch.float64,
        "torch.float64": torch.float64,
        "double": torch.float64,
        "float16": torch.float16,
        "torch.float16": torch.float16,
        "half": torch.float16,
    }
    try:
        return mapping[text]
    except KeyError as exc:
        raise ValueError(f"Unsupported dtype value: {value!r}.") from exc


def _parse_shape_value(value: Any) -> tuple[int, ...] | None:
    """Parse shape-like manifest/config values into integer tuples."""

    if value is None:
        return None
    if isinstance(value, str):
        text = value.strip()
        if not text:
            return None
        try:
            parsed = ast.literal_eval(text)
        except (SyntaxError, ValueError) as exc:
            raise ValueError(f"Could not parse shape string: {value!r}.") from exc
        value = parsed
    if isinstance(value, np.ndarray):
        value = value.tolist()
    if isinstance(value, Sequence):
        try:
            return tuple(int(item) for item in value)
        except (TypeError, ValueError) as exc:
            raise ValueError(f"Invalid shape value: {value!r}.") from exc
    raise ValueError(f"Unsupported shape value: {value!r}.")


def _resolve_expected_shape(
    keypoint_set: str,
    expected_shape: Sequence[int] | str | None,
) -> tuple[int, ...]:
    """Resolve the canonical expected graph tensor shape for one keypoint set."""

    if expected_shape is not None:
        parsed = _parse_shape_value(expected_shape)
        if parsed is None:
            raise ValueError("expected_shape must not be empty when provided.")
        return parsed
    try:
        return DEFAULT_EXPECTED_SHAPES[keypoint_set]
    except KeyError as exc:
        raise KeyError(
            f"Unsupported keypoint_set {keypoint_set!r}. Expected one of {sorted(DEFAULT_EXPECTED_SHAPES)}."
        ) from exc


def _dataset_marker_candidate(path_text: str, project_root: Path) -> Path | None:
    """Map an absolute foreign path back under the local project root when possible."""

    normalized = path_text.replace("\\", "/")
    if DEFAULT_DATASET_ROOT_MARKER not in normalized:
        return None
    suffix = normalized.split(DEFAULT_DATASET_ROOT_MARKER, 1)[1]
    return project_root / "data" / "datasets" / "WLASL" / Path(suffix)


def _data_root_candidate(path_text: str, data_root: Path) -> Path | None:
    """Map one absolute foreign path back under a configured skeleton branch root."""

    normalized = path_text.replace("\\", "/")
    marker = DEFAULT_SKELETON_BRANCH_MARKER
    if marker in normalized:
        suffix = normalized.split(marker, 1)[1]
        return data_root / Path(suffix)

    if DEFAULT_DATASET_ROOT_MARKER in normalized:
        suffix = normalized.split(DEFAULT_DATASET_ROOT_MARKER, 1)[1]
        return data_root / Path(suffix)

    raw_path = Path(path_text)
    if not raw_path.is_absolute():
        return data_root / raw_path
    return None


def resolve_graph_tensor_path(
    path_text: str | Path,
    *,
    project_root: str | Path | None = None,
    data_root: str | Path | None = None,
) -> Path:
    """Resolve one graph tensor path across local, HF, and Kaggle workspaces."""

    text = _safe_str(path_text).strip()
    if not text:
        raise ValueError("graph_tensor_path is empty.")

    project_path = Path(project_root or Path.cwd()).resolve()
    data_path = Path(data_root).resolve() if data_root is not None else None
    raw_path = Path(text)

    candidates: list[Path] = []
    if raw_path.is_absolute():
        candidates.append(raw_path)
    else:
        candidates.append(project_path / raw_path)
        if data_path is not None:
            candidates.append(data_path / raw_path)

    marker_candidate = _dataset_marker_candidate(text, project_path)
    if marker_candidate is not None:
        candidates.append(marker_candidate)

    if data_path is not None:
        remapped_data_candidate = _data_root_candidate(text, data_path)
        if remapped_data_candidate is not None:
            candidates.append(remapped_data_candidate)

    seen: set[str] = set()
    for candidate in candidates:
        normalized = str(candidate.resolve(strict=False))
        if normalized in seen:
            continue
        seen.add(normalized)
        if candidate.exists():
            return candidate.resolve()

    candidate_text = ", ".join(str(Path(candidate)) for candidate in candidates)
    raise FileNotFoundError(
        "Could not resolve graph tensor path "
        f"{text!r}. Tried: {candidate_text or '<none>'}."
    )


def build_label_maps_from_manifest(
    manifest: pd.DataFrame,
    *,
    logger=LOGGER,
) -> tuple[dict[int, str], dict[str, int]]:
    """Build ``class_id -> gloss`` and ``gloss -> class_id`` mappings from one manifest."""

    if manifest.empty:
        return {}, {}

    require_columns(manifest, ("class_id", "gloss"), name="skeleton_manifest")
    working = manifest.loc[:, ["class_id", "gloss"]].copy()
    working["class_id"] = working["class_id"].apply(lambda value: _parse_int(value, "class_id"))
    working["gloss"] = working["gloss"].fillna("").astype(str)

    id_to_gloss: dict[int, str] = {}
    gloss_to_id: dict[str, int] = {}
    duplicate_glosses: list[str] = []

    for class_id, group in working.groupby("class_id", sort=True):
        glosses = sorted({value.strip() for value in group["gloss"].tolist() if value.strip()})
        gloss = glosses[0] if glosses else ""
        id_to_gloss[int(class_id)] = gloss
        if gloss:
            existing = gloss_to_id.get(gloss)
            if existing is not None and existing != int(class_id):
                duplicate_glosses.append(gloss)
            else:
                gloss_to_id[gloss] = int(class_id)

    class_ids = sorted(id_to_gloss)
    expected_contiguous = list(range(min(class_ids), max(class_ids) + 1)) if class_ids else []
    if class_ids and class_ids != expected_contiguous:
        logger.warning(
            "class_id values are not contiguous. min=%s max=%s unique=%s",
            min(class_ids),
            max(class_ids),
            len(class_ids),
        )
    if duplicate_glosses:
        logger.warning(
            "Found gloss values mapped to multiple class IDs: %s",
            ", ".join(sorted(set(duplicate_glosses))),
        )
    return id_to_gloss, gloss_to_id


def load_skeleton_train_config(
    config_path: str | Path,
    *,
    project_root: str | Path | None = None,
) -> dict[str, Any]:
    """Load and normalize one training-oriented skeleton dataset config."""

    config = read_yaml(config_path)
    dataset_cfg = config.get("dataset", {})
    dataloader_cfg = config.get("dataloader", {})
    graph_cfg = config.get("graph", {})

    project_path = Path(project_root or Path.cwd()).resolve()

    manifests_cfg = dataset_cfg.get("manifests", {})
    resolved_manifests = {
        split: (
            Path(path_value)
            if Path(path_value).is_absolute()
            else (project_path / Path(path_value))
        )
        for split, path_value in manifests_cfg.items()
    }

    data_root_value = dataset_cfg.get("data_root")
    data_root = None
    if data_root_value:
        data_root_path = Path(data_root_value)
        data_root = data_root_path if data_root_path.is_absolute() else (project_path / data_root_path)

    return {
        "config_path": Path(config_path),
        "project_root": project_path,
        "dataset": {
            "name": dataset_cfg.get("name", "WLASL"),
            "subset": dataset_cfg.get("subset", "nslt100"),
            "data_root": data_root,
            "keypoint_set": str(dataset_cfg.get("keypoint_set", "selected_27")),
            "num_classes": int(dataset_cfg.get("num_classes", 100)),
            "num_channels": int(dataset_cfg.get("num_channels", 3)),
            "target_num_frames": int(dataset_cfg.get("target_num_frames", 150)),
            "num_persons": int(dataset_cfg.get("num_persons", 1)),
            "expected_shape": _resolve_expected_shape(
                str(dataset_cfg.get("keypoint_set", "selected_27")),
                dataset_cfg.get("expected_shape"),
            ),
            "manifests": resolved_manifests,
            "return_metadata": bool(dataset_cfg.get("return_metadata", True)),
            "strict_shape_check": bool(dataset_cfg.get("strict_shape_check", True)),
        },
        "dataloader": {
            "batch_size": int(dataloader_cfg.get("batch_size", 16)),
            "num_workers": int(dataloader_cfg.get("num_workers", 0)),
            "pin_memory": bool(dataloader_cfg.get("pin_memory", False)),
            "shuffle_train": bool(dataloader_cfg.get("shuffle_train", True)),
        },
        "graph": {
            "layout": str(graph_cfg.get("layout", dataset_cfg.get("keypoint_set", "selected_27"))),
            "strategy": str(graph_cfg.get("strategy", "spatial")),
            "add_self_links": bool(graph_cfg.get("add_self_links", True)),
            "normalize_adjacency": bool(graph_cfg.get("normalize_adjacency", True)),
        },
    }


class SkeletonGraphDataset(Dataset):
    """Load precomputed skeleton graph tensors and integer class labels."""

    def __init__(
        self,
        *,
        manifest_path: str | Path,
        project_root: str | Path | None = None,
        data_root: str | Path | None = None,
        keypoint_set: str = "selected_27",
        split: str | None = None,
        expected_shape: Sequence[int] | str | None = None,
        num_classes: int | None = None,
        return_metadata: bool = True,
        dtype: Any = torch.float32,
        strict_shape_check: bool = True,
        strict_path_check: bool | None = None,
        limit: int | None = None,
        logger=LOGGER,
    ) -> None:
        self.manifest_path = Path(manifest_path)
        self.project_root = Path(project_root or Path.cwd()).resolve()
        self.data_root = Path(data_root).resolve() if data_root is not None else None
        self.keypoint_set = str(keypoint_set)
        self.split = _normalize_split(split) if split is not None else None
        self.expected_shape = _resolve_expected_shape(self.keypoint_set, expected_shape)
        self.num_classes = num_classes
        self.return_metadata = bool(return_metadata)
        self.dtype = _coerce_torch_dtype(dtype)
        self.strict_shape_check = bool(strict_shape_check)
        self.strict_path_check = self.strict_shape_check if strict_path_check is None else bool(strict_path_check)
        self.limit = limit
        self.logger = logger

        if self.split is not None and self.split not in ALLOWED_SPLITS:
            raise ValueError(f"Unsupported split: {split!r}. Expected one of {ALLOWED_SPLITS}.")
        if self.num_classes is not None and self.num_classes <= 0:
            raise ValueError("num_classes must be positive when provided.")

        self.manifest = self._load_manifest()
        self.id_to_gloss, self.gloss_to_id = build_label_maps_from_manifest(self.manifest, logger=self.logger)
        self.records = self._build_records()
        if not self.records:
            raise ValueError(
                f"No usable samples were found in manifest {self.manifest_path} after filtering."
            )

    @classmethod
    def from_config(
        cls,
        config: str | Path | dict[str, Any],
        *,
        split: str | None = None,
        return_metadata: bool | None = None,
        limit: int | None = None,
        dtype: Any = torch.float32,
        logger=LOGGER,
    ) -> "SkeletonGraphDataset":
        """Instantiate a dataset from one train config dict or YAML file."""

        if isinstance(config, (str, Path)):
            resolved = load_skeleton_train_config(config)
        elif isinstance(config, dict):
            resolved = config
        else:
            raise TypeError(f"Unsupported config type: {type(config)!r}.")

        dataset_cfg = resolved.get("dataset", {})
        manifest_map = dataset_cfg.get("manifests", {})
        dataset_split = _normalize_split(split) if split is not None else None
        selected_split = dataset_split or "train"
        try:
            manifest_path = manifest_map[selected_split]
        except KeyError as exc:
            raise KeyError(f"Manifest for split {selected_split!r} is missing from config.") from exc

        return cls(
            manifest_path=manifest_path,
            project_root=resolved.get("project_root"),
            data_root=dataset_cfg.get("data_root"),
            keypoint_set=str(dataset_cfg.get("keypoint_set", "selected_27")),
            split=dataset_split,
            expected_shape=dataset_cfg.get("expected_shape"),
            num_classes=dataset_cfg.get("num_classes"),
            return_metadata=dataset_cfg.get("return_metadata", True)
            if return_metadata is None
            else return_metadata,
            dtype=dtype,
            strict_shape_check=bool(dataset_cfg.get("strict_shape_check", True)),
            limit=limit,
            logger=logger,
        )

    def _load_manifest(self) -> pd.DataFrame:
        """Read and normalize the skeleton manifest CSV."""

        if not self.manifest_path.exists():
            raise FileNotFoundError(f"Manifest file does not exist: {self.manifest_path}")

        dtype_map = {
            column: "string"
            for column in SKELETON_INPUT_MANIFEST_COLUMNS
            if column not in {"class_id", "num_frames_original", "num_frames_output", "num_source_keypoints",
                              "num_selected_keypoints", "num_channels", "num_persons", "confidence_scale"}
        }
        manifest = read_csv(self.manifest_path, dtype=dtype_map)
        require_columns(manifest, REQUIRED_DATASET_COLUMNS, name="skeleton_manifest")

        working = manifest.copy()
        working["status"] = working["status"].fillna("").astype(str).str.strip().str.lower()
        working["split"] = working["split"].fillna("").astype(str).str.strip().str.lower()
        working["graph_tensor_path"] = working["graph_tensor_path"].fillna("").astype(str).str.strip()
        working["sample_id"] = working["sample_id"].fillna("").astype(str).str.strip()
        working["video_id"] = working["video_id"].fillna("").astype(str).str.strip()
        working["gloss"] = working["gloss"].fillna("").astype(str).str.strip()
        working["keypoint_set"] = working.get("keypoint_set", pd.Series("", index=working.index)).fillna("").astype(str)

        working = working[working["status"] == "ok"].copy()
        if self.split is not None:
            working = working[working["split"] == self.split].copy()
        working = working[working["graph_tensor_path"] != ""].copy()
        working = working.reset_index(drop=True)

        if self.limit is not None:
            working = working.head(int(self.limit)).reset_index(drop=True)
        return working

    def _validate_manifest_row_shape(self, row: pd.Series) -> None:
        """Validate one manifest row's declared tensor shape."""

        tensor_shape_text = _safe_str(row.get("tensor_shape")).strip()
        if not tensor_shape_text:
            return
        manifest_shape = _parse_shape_value(tensor_shape_text)
        if manifest_shape is None:
            return
        if manifest_shape != self.expected_shape:
            message = (
                f"Manifest tensor_shape {manifest_shape} does not match expected_shape "
                f"{self.expected_shape} for sample_id={row.get('sample_id')}."
            )
            if self.strict_shape_check:
                raise ValueError(message)
            self.logger.warning(message)

    def _build_records(self) -> list[SkeletonSampleRecord]:
        """Resolve usable records from the filtered manifest."""

        records: list[SkeletonSampleRecord] = []
        skipped = 0

        for manifest_index, row in self.manifest.iterrows():
            row_keypoint_set = _safe_str(row.get("keypoint_set")).strip()
            if row_keypoint_set and row_keypoint_set != self.keypoint_set:
                skipped += 1
                continue

            try:
                self._validate_manifest_row_shape(row)
                class_id = _parse_int(row.get("class_id"), "class_id")
                if self.num_classes is not None and not 0 <= class_id < int(self.num_classes):
                    raise ValueError(
                        f"class_id {class_id} is outside the expected range [0, {int(self.num_classes) - 1}]."
                    )
                graph_tensor_path = resolve_graph_tensor_path(
                    row.get("graph_tensor_path", ""),
                    project_root=self.project_root,
                    data_root=self.data_root,
                )
            except Exception as exc:
                if self.strict_path_check:
                    raise
                skipped += 1
                self.logger.warning(
                    "Skipping manifest row index=%s sample_id=%s: %s",
                    manifest_index,
                    row.get("sample_id"),
                    exc,
                )
                continue

            records.append(
                SkeletonSampleRecord(
                    sample_id=_safe_str(row.get("sample_id")).strip(),
                    video_id=_safe_str(row.get("video_id")).strip(),
                    gloss=_safe_str(row.get("gloss")).strip(),
                    class_id=class_id,
                    split=_normalize_split(row.get("split")),
                    graph_tensor_path=graph_tensor_path,
                    manifest_index=int(manifest_index),
                    instance_uid=_safe_str(row.get("instance_uid")).strip(),
                    keypoint_set=row_keypoint_set or self.keypoint_set,
                    original_path_text=_safe_str(row.get("graph_tensor_path")).strip(),
                    notes=_safe_str(row.get("notes")).strip(),
                )
            )

        if skipped:
            self.logger.warning("Skipped %s manifest rows while building skeleton dataset.", skipped)
        return records

    def __len__(self) -> int:
        """Return the number of resolved skeleton samples."""

        return len(self.records)

    def _load_graph_tensor(self, path: Path) -> np.ndarray:
        """Load one graph tensor ``.npz`` file."""

        with np.load(path, allow_pickle=False) as payload:
            if "data" in payload:
                array = payload["data"]
            elif "tensor" in payload:
                array = payload["tensor"]
            else:
                raise KeyError(
                    f"Graph tensor file {path} does not contain 'data' or fallback key 'tensor'."
                )

        graph_tensor = np.asarray(array, dtype=np.float32)
        if self.strict_shape_check and tuple(graph_tensor.shape) != self.expected_shape:
            raise ValueError(
                f"Graph tensor shape mismatch for {path}: expected {self.expected_shape}, "
                f"got {tuple(graph_tensor.shape)}."
            )
        return graph_tensor

    def __getitem__(self, index: int) -> dict[str, Any] | tuple[torch.Tensor, int]:
        """Return one graph tensor sample and its integer class label."""

        record = self.records[index]
        graph_tensor = self._load_graph_tensor(record.graph_tensor_path)
        data = torch.as_tensor(graph_tensor, dtype=self.dtype)
        label = int(record.class_id)

        if not self.return_metadata:
            return data, label

        return {
            "data": data,
            "label": label,
            "sample_id": record.sample_id,
            "video_id": record.video_id,
            "gloss": record.gloss,
            "class_id": label,
            "split": record.split,
            "path": str(record.graph_tensor_path),
        }


def skeleton_collate_fn(batch: Sequence[dict[str, Any] | tuple[torch.Tensor, int]]) -> dict[str, Any]:
    """Collate skeleton samples into batched tensors plus optional metadata."""

    if not batch:
        raise ValueError("Batch is empty.")

    first = batch[0]
    if isinstance(first, tuple):
        data_tensors = [item[0] for item in batch]
        labels = [int(item[1]) for item in batch]
        return {
            "data": torch.stack(data_tensors, dim=0),
            "labels": torch.as_tensor(labels, dtype=torch.long),
        }

    data_tensors = [item["data"] for item in batch]
    labels = [int(item["label"]) for item in batch]
    metadata = [
        {
            "sample_id": item["sample_id"],
            "video_id": item["video_id"],
            "gloss": item["gloss"],
            "class_id": int(item["class_id"]),
            "split": item["split"],
            "path": item["path"],
        }
        for item in batch
    ]
    return {
        "data": torch.stack(data_tensors, dim=0),
        "labels": torch.as_tensor(labels, dtype=torch.long),
        "metadata": metadata,
    }


__all__ = [
    "SkeletonGraphDataset",
    "SkeletonSampleRecord",
    "build_label_maps_from_manifest",
    "load_skeleton_train_config",
    "resolve_graph_tensor_path",
    "skeleton_collate_fn",
]
